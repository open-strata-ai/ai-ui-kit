# ADR-0005: OQ-5: Is the library version strictly released simultaneously with platform v1.4.0, or is it independent of SemVer?
...
Is the library version strictly released simultaneously with platform v1.4.0, or is it independent of SemVer? How does the `openstrata.interfaceVersion` namespace align with §16.1's class 15 SPI ports (this library is not an SPI instance)? | Remarks: §16.1 Each capability interface is independent of SemVer; this library needs to define its own interface version number.
...
This ADR solidifies "OQ-5: Is the library version strictly synchronized with the platform v1.4.0, or is it independent of SemVer?" into an architectural decision record and included in `design/adr/` for continuous tracking. This issue stems from the `design/DESIGN.md` §10 open issue and is still open.
