...
This ADR solidifies "OQ-7: Stability strategy for visual regression baseline under cross-OS/cross-browser font differences (Chromatic vs self-hosted)?" into an architectural decision record and included in `design/adr/` for continuous tracking. This issue stems from the `design/DESIGN.md` §10 open issue and is still open.
