# ADR-0008: OQ-8: Is React 19 compatibility and antd v6 upgrade path included in v1.4.x planning?
...
Will React 19 compatibility and antd v6 upgrade route be included in the v1.4.x plan? | Note: Current baseline React 18 + antd v5 (§2533). ---
...
This ADR solidifies "OQ-8: React 19 compatibility and antd v6 upgrade route into v1.4.x planning?" into an architectural decision record and includes it in `design/adr/` for continuous tracking. This issue stems from the `design/DESIGN.md` §10 open issue and is still open.
