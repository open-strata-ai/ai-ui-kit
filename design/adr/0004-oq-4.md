# ADR-0004: OQ-4: Mermaid's rendering and hydration strategy under SSR (Next.js, §101 front-end using Next.js)?
...
Mermaid's rendering and hydration strategy under SSR (Next.js, §101 frontend with Next.js)? Is client-side rendering and lazy loading the default? | Note: The boot portal/management portal is CSR (Vite), but it needs to be compatible with the potential SSR host.
...
This ADR will solidify "OQ-4: Mermaid's rendering and hydration strategy under SSR (Next.js, §101 front-end uses Next.js)?" into an architectural decision record and include it in `design/adr/` for continuous tracking. This issue stems from the `design/DESIGN.md` §10 open issue and is still open.
...
- Associated architecture documents §101 (as a basis for decision-making and a source of consistency verification).
