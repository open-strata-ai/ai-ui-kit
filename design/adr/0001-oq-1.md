...
Should the official npm package name be `@openstrata/ai-ui-kit` or the architecture documentation example `@ai-infra/ui-kit`? Do you want to keep the alias? | Note: §4.1.2 The example uses `@ai-infra/ui-kit`; the repo is identified as `ai-ui-kit`. It is recommended to refer to `@openstrata/ai-ui-kit` and mark the alias in `specs/SPECS.md`.
...
This ADR solidifies "OQ-1: Should the official npm package name be `@openstrata/ai-ui-kit` or the architecture document example `@ai-infra/ui-kit`?" into an architectural decision record and include it in `design/adr/` for continuous tracking. This issue stems from the `design/DESIGN.md` §10 open issue and is still open.
